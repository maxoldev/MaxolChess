...
struct PossibleMovesTest {
    ...
    @Test func pawnMovesOnEmptyBoard() async throws {
        var pos = Position(try #require(Board(pieces: (Piece(.white, .pawn), "e2"))), sideToMove: .white)
        ...
        pos = Position(try #require(Board(pieces: (Piece(.black, .pawn), "e7"))), sideToMove: .black)
        ...
        pos = Position(try #require(Board(pieces: (Piece(.white, .pawn), "e3"))), sideToMove: .white)
        moves = moveGen.generateAllMoves(pos, parentMoveId: nil)
        #expect(moves.count == 1)
        #expect((moves[0] as? RepositionMove)?.from == "e3")
        #expect((moves[0] as? RepositionMove)?.to == "e4")

        pos = Position(try #require(Board(pieces: (Piece(.white, .pawn), "e4"))), sideToMove: .white)
        moves = moveGen.generateAllMoves(pos, parentMoveId: nil)
        #expect(moves.count == 1)
    ...
    @Test func knightMovesOnEmptyBoard() async throws {
        let pos = Position(try #require(Board(pieces: (Piece(.white, .knight), "e5"))), sideToMove: .white)
    ...
    @Test func kingMovesOnEmptyBoard() async throws {
        let pos = Position(try #require(Board(pieces: (Piece(.white, .king), "e5"))), sideToMove: .white)
        let moves = moveGen.generateAllMoves(pos, parentMoveId: nil)
